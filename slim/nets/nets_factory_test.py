...
from slim.nets import slim.nets_factory
...
class NetworksTest(tf.test.TestCase):
  ...
  def testGetNetworkFnFirstHalf(self):
    batch_size = 5
    num_classes = 1000
    for net in list(slim.nets_factory.networks_map.keys())[:10]:
      with tf.Graph().as_default() as g, self.test_session(g):
        net_fn = slim.nets_factory.get_network_fn(net, num_classes=num_classes)
